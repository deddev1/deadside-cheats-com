...
const USER_SCREENSHOTS = [
	{
		...
		file: 'deadside-esp-player-tags.webp',
	},
	{
		source:
		...
		file: 'deadside-wallhack-skeleton.webp',
	},
	{
		source:
		...
		file: 'deadside-aimbot-sniper.webp',
	},
	{
		source:
		...
		file: 'deadside-aimbot-skeleton.webp',
	},
	{
		source:
	...
	},
	{
		...
		file: 'deadside-cheats-combat.webp',
	},
];
...
const REMOVE_PREFIXES = [
	...
	'deadside-cheats-cover',
	'deadside-cheats-esp-wallhack',
	'deadside-cheats-hero',
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'deadside-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `deadside-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'deadside-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'deadside-cheats-logo.webp');
