...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'deadside-cheats-logo.png'), logoPng);
	console.log('Wrote public/images/deadside-cheats-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'deadside-cheats-logo.webp'), logoWebp);
	console.log('Wrote public/images/deadside-cheats-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		...
		short_name: 'Deadside Cheats',
		description: 'Undetected deadside cheats — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
