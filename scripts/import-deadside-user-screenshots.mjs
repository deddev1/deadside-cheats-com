/**
 * Import user-provided Deadside gameplay screenshots (local PNGs).
 * Writes /images/deadside-screenshot-01.webp … 08.webp + -480w / -960w variants
...
const USER_SOURCES = [
	...
	},
	{
		...
		label: 'Deadside third-person gameplay on Windows PC',
	},
	{
...
const LEGACY_MAP = {
	'deadside-screenshot-01': ['deadside-cheats-esp.webp', 'deadside-esp-player-tags.webp'],
	...
	'deadside-screenshot-06': ['deadside-extract-fight.webp', 'deadside-loot-run-combat.webp'],
	'deadside-screenshot-07': ['deadside-loot-run-mode.webp'],
	'deadside-screenshot-08': [],
};
...
await mkdir(path.join(ROOT, 'scripts/assets/deadside-screenshots'), { recursive: true });

const sourcePaths = [];
for (let i = 0; i < USER_SOURCES.length; i += 1) {
	const src = path.join(assetsDir, USER_SOURCES[i].file);
	const saved = path.join(ROOT, 'scripts/assets/deadside-screenshots', `source-${i + 1}.png`);
...
const reviewsCanonical = canonicalBySlot['deadside-screenshot-04'];
