/** SERP title helpers — min 45 chars, max 60 chars / ~568px (Screaming Frog approx). */
...
export function titlePixelWidth(text: string): number {
	...
	for (const ch of text) {
		...
		else if (ch >= '0' && ch <= '9') px += 6;
		else if ((ch.codePointAt(0) ?? 0) > 0x2e80) px += 14;
		else px += 7;
	}
	return px;
}

export function fitsTitleLimits(text: string): boolean {
	return (
...
export function hasRichKeywords(text: string): boolean {
...
export function expandTitleMin(text: string): string {
	let s = text.trim();
...
export function clampTitleMax(text: string): string {
...
export function finalizeTitle(text: string): string {
